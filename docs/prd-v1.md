...
- ***`internal/slug`*** — Reserved-words constant slice + `Generate() → string` (8-char base62) + `Validate(s) → error`. The slug-collision retry loop lives here, not in `store`, because reserved-words check + retry-on-collision is one concern with one home.
...
- ***`internal/server/store`*** — Two sub-packages behind a single interface boundary: `postgres` (documents-row CRUD: `InsertPending`, `Commit`, `GetBySlug`, `GetByIdempotencyKey`) and `r2` (presigned-PUT minting with `Content-Length` + `x-amz-checksum-sha256` pinned into the V4 signature, `HEAD` verification, raw blob fetch for SSR). All SQL inline; no ORM. R2 client wraps the AWS SDK v2 with mdfly-specific helpers.
...
- ***`internal/server/store/postgres`*** — integration tests against testcontainer Postgres + migrations applied. Cover: `InsertPending` idempotency under `ON CONFLICT (idempotency_key) DO NOTHING` with the same key (twice) returns the same row; `Commit` is a no-op idempotency when the row is already `published`; partial-index `documents_pending_gc_idx` exists and is used by an `EXPLAIN ANALYZE` of the GC query.
- ***`internal/server/store/r2`*** — integration tests against minio. Cover: presigned PUT with correct `Content-Length` + `x-amz-checksum-sha256` succeeds; presigned PUT with wrong checksum is rejected by minio; HEAD-verify on a present blob succeeds; HEAD-verify on a missing blob returns the expected `BlobMissing` error.
