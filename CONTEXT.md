...
- **init**: CLI sends `{idempotency_key, bundle, edit_token?, requested_slug?}` (where `bundle` is `{root_hash, files:[{path,hash,size}]}`); server INSERTs a `documents` row with `status='pending'` (using `ON CONFLICT (idempotency_key) DO NOTHING` to be retry-safe), stores `SHA256(edit_token)` if present, returns `{slug, presigned_urls}` where `presigned_urls` maps each content hash to its presigned PUT URL.
...
|---|---|---|---|---|---|
| `api.mdfly.dev/v1/publish/init` | POST | Bearer (session) OR Edit Token OR none (anon-new) | `{idempotency_key, bundle:{root_hash, files:[{path,hash,size}]}, edit_token?, requested_slug?}` | `200 {slug, presigned_urls:{hash:url}, inline_accept:bool}` | `400` bad bundle, `401` bad token, `403` `--slug` w/o session, `409` slug taken or reserved, `413` bundle too big, `422` idempotency-key reused with different payload, `429` rate-limit |
...
### Bundle Manifest
Per-Document `files_by_path` keyed by each file's **bundle-relative logical path** (`index.md`, `b.md`, `imgs/logo.png`), mapping to its [[Content Hash]] and size, plus a `root_path` field naming which entry is the Root markdown. The CLI sends the wire form (`bundle:{root_hash, files:[{path,hash,size}]}`); the server re-indexes it by path on init, since SSR resolves asset references by logical path. Canonical copy lives in Postgres (`documents.manifest jsonb`). Markdown files are stored byte-identical to what the user wrote; the Manifest is what lets `![](./logo.png)` inside a markdown file resolve to `cdn.mdfly.dev/documents/<slug>/<hash>.png` at render time.
