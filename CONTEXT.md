...
### Bundle
The complete set of files uploaded for one Publish: the [[Root]] file (any type) + its reachable [[Asset]]s. Without `-r` the Bundle is the Root plus its directly-referenced non-`.md` assets only (a binary Root has none, so its Bundle is a single file — no walk); with `-r` the Bundle also includes transitively-linked `.md` files and their assets. Subject to Bundle Limits.
