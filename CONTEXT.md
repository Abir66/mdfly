...
- **init**: CLI sends `{idempotency_key, manifest, edit_token?, requested_slug?}`; server INSERTs a `documents` row with `status='pending'` (using `ON CONFLICT (idempotency_key) DO NOTHING` to be retry-safe), stores `SHA256(edit_token)` if present, returns `{slug, missing_hashes, presigned_urls}`.
- **R2 upload**: CLI PUTs missing blobs directly to R2 using presigned URLs that pin `Content-Length` and `x-amz-checksum-sha256`, so R2 rejects mismatched bytes at upload.
...
### Asset
Any non-root file referenced by a Document — images, additional `.md` files, etc. — that is resolved and uploaded alongside the root. Resolution follows reachable relative paths (no sandbox), gated by a total size cap and a total file count cap per Bundle.
...
|---|---|---|---|---|---|
| `api.mdfly.dev/v1/publish/init` | POST | Bearer (session) OR Edit Token OR none (anon-new) | `{idempotency_key, manifest:{root,files:[{path,hash,size}]}, edit_token?, requested_slug?}` | `200 {slug, missing_hashes:[...], presigned_urls:{hash:url}, inline_accept:bool}` | `400` bad manifest, `401` bad token, `403` `--slug` w/o session, `409` slug taken or reserved, `413` bundle too big, `422` idempotency-key reused with different payload, `429` rate-limit |
...
### Root
The single entry-point markdown file of a Bundle, identified by its bundle-relative path in the [[Bundle Manifest]]. Rendered when viewer hits the bare Document URL.
...
### Bundle Manifest
Per-Document mapping from each file's **bundle-relative logical path** (`index.md`, `b.md`, `imgs/logo.png`) to its [[Content Hash]] and size, plus a `root` field naming which entry is the Root markdown. Canonical copy lives in Postgres (`documents.bundle_manifest jsonb`). Markdown files are stored byte-identical to what the user wrote; the Manifest is what lets `![](./logo.png)` inside a markdown file resolve to `cdn.mdfly.dev/documents/<slug>/<hash>.png` at render time.

Manifest is consumed exclusively server-side. The Go backend reads it from Postgres at view time, fetches the requested markdown blob from R2, rewrites logical references (assets → `cdn.mdfly.dev/documents/<slug>/<hash>.<ext>`; cross-md links → `mdfly.dev/<slug>/<path>` for humans or `mdfly.dev/llm/<slug>/<path>.md` for agents), then either SSRs the result to HTML (human routes) or returns the resolved markdown bytes verbatim (LLM routes). The Manifest is **never** sent to the client. See ADR-0018 (supersedes ADR-0009) and the amended ADR-0001.
...
### Preview Metadata
Server-extracted OpenGraph tags emitted in the SSR'd HTML at view time (ADR-0018). Resolution order: frontmatter `title` / `description` / `image` if present; otherwise inferred — title from first H1, description from first paragraph (truncated), image from first referenced image Asset. Computed once at Publish/Update commit time and cached as `title` / `excerpt` / `og_image_hash` columns on `documents` so the HTML render does not need to re-parse markdown just to fill the `<head>`. Enables link unfurl in chat clients.
...
### Repo Layout
Single Go monorepo at `github.com/mdfly/mdfly`, single `go.mod`, two Go binaries (`cmd/mdfly` CLI + `cmd/mdfly-server` backend), shared Go code in compiler-private `internal/` packages. Reserves `apps/` for future non-Go deliverables (`apps/landing/` v1.5 marketing, `apps/dashboard/` v2 React SPA). Root-level: `migrations/` (golang-migrate SQL), `deploy/` (Cloudflare + DO + R2 IaC), `scripts/`, `docs/adr/`, `testdata/`, `Dockerfile` (server), `.goreleaser.yaml` (CLI), `Makefile`. Internal packages: `slug`, `manifest`, `markdown`, `api` (wire-type source of truth + future TS codegen), `auth`, `store`, `ssr`, `llm`, `purge`. SSR templates `//go:embed`'d in `internal/ssr/`. The wire protocol's volatility at v1 (3-phase publish, idempotency-key, optimistic-concurrency) makes atomic cross-binary PRs valuable; polyrepo would force every wire change through a 3-PR shared-module-tag dance with real version-skew hazard. See ADR-0022.
