...
### API Surface
HTTP surface mounted under `mdfly.devapi.mdfly.dev/v1/`. The `v1` prefix is cheap version insurance: lets a future `v2` endpoint cut without touching siblings. `Update` has its **own** `init` + `commit` pair (`/v1/update/init`, `/v1/update/commit`), separate from `publish`'s (ADR-0027, reversing ADR-0011's original "update reuses publish" recommendation): create and update diverge at the row op (INSERT-pending vs atomic-overwrite) and the auth model (anon-none vs Edit-Token-required), so each gets a single-responsibility wire while still sharing the leaf upload/verify machinery. The update path is **stateless** — `update/init` verifies the Edit Token, diffs the manifest, and presigns only new `(hash, ext)` blobs without writing anything; `update/commit` resends the manifest, HEAD-verifies blobs, and does one atomic optimistic UPDATE. It carries **no idempotency-key** (ADR-0013 is publish-only): the new manifest hash is the idempotency token via a three-way commit compare, and optimistic concurrency (ADR-0012) is the `parent_manifest_hash` guard on that UPDATE. The inline-blob fast-path (ADR-0011) is **deferred in v1** — the `inline_blobs` field on `commit` and `inline_accept` in the `init` response remain reserved but unused (server never sets `inline_accept: true`, CLI always uploads via presigned PUT). The view path's `GET api.mdfly.dev/v1/documents/:slug` is **unauthenticated** — visibility is public-only by ADR-0008; privacy is slug obscurity. State-changing endpoints always require the token in the `Authorization` header, never just a cookie — eliminates CSRF (ADR-0016).
...
### Anonymous Document
Document published without a logged-in user. URL form: `mdfly.dev/<slug>` (random slug). Ownership proven by a secret Edit Token returned at publish time. Auto-expires 30 days after Publish (or last Update); at expiry the lifecycle GC flips the row to `status='expired'` (serves 410, like a Delete) and deletes its blobs after a grace window (ADR-0030). Can be promoted to Owned via [[Claim]].
...
**No bare-command shortcut.** Every invocation names its verb — `mdfly publish <file>` or `mdfly update <file>`, never a bare `mdfly <file>`. The verb is always explicit so create-vs-modify intent is legible from the command itself and there is no on-disk heuristic that could silently pick the wrong path (ADR-0014).
...
**Wire (ADR-0027).** Update has its own two-phase endpoints, separate from publish and carrying **no idempotency-key**:
...
### Delete
Owner-triggered removal of a Document via `mdfly delete <slug-or-file>` (a file arg resolves through [[Local State]], same slug-disambiguation rule as [[Update]]). Soft-deletes the Postgres row (flips `status` to `'deleted'` and stamps `deleted_at`, slug stays reserved and not reusable); R2 blob deletion is deferred to a server-side GC (out of the delete request's path) that removes the whole `documents/<slug>/` prefix after a grace window (ADR-0030). The URL responds **410 Gone** thereafter — distinct from 404 (never existed). On success — or on a `404`/`410` meaning the slug was already gone — the CLI prunes the local [[Local State]] record (distinct from [[Remove]], which only touches local state and never calls the server). Expiry of an Anonymous Document reaches the same 410 outcome via the distinct `status='expired'` state (ADR-0030).
...
- **Soft-delete by `status='deleted'` + `deleted_at TIMESTAMPTZ` stamp**, never row removal. A `'deleted'` row serves 410 and the slug stays reserved forever per ADR-0002; its R2 blobs are deleted later by a server-side GC (not on the delete request path, ADR-0030). `users.deleted_at IS NOT NULL` is the manual user-delete tombstone; under ADR-0017 this is a ticket-driven process, not self-service.
...
- **`documents.status`** carries `'pending'`, `'published'`, `'deleted'`, `'expired'`, and `'abandoned'`. A delete flips to `'deleted'` (+ stamps `deleted_at`); the lifecycle GC flips an anon row past `expires_at` to `'expired'` and a never-committed `pending` row past its 1h grace to `'abandoned'` (ADR-0030). `'deleted'`/`'expired'` serve **410**, `'abandoned'` serves **404**. Stored as `TEXT` with a `CHECK` constraint, not native `ENUM`, because extending `TEXT + CHECK` is a trivial migration (S34's `0002` does exactly this) whereas `ALTER TYPE ... ADD VALUE` on an enum is fiddly.
- **`documents.blobs_deleted_at TIMESTAMPTZ NULLABLE`** records that the row's R2 blobs (`documents/<slug>/` prefix) have been deleted by the lifecycle GC; `NULL` means blobs are still present. The GC deletes blobs after a 24h grace for `'deleted'`/`'expired'` and immediately for `'abandoned'` (ADR-0030).
