...
**`?up=N` URL scheme** — a key above the project root (`../x.md`) cannot be a path segment (browsers/Cloudflare/Go `ServeMux` strip `..` via RFC 3986 dot-segment removal before the server sees it). Such keys are addressed with a surviving query param: `/<slug>/<tail>?up=N` where `N` = count of leading `../` and `tail` = the rest with `.md` stripped. Server reconstructs `key = strings.Repeat("../", N) + tail (+ ".md")`. No server-side cap on `N`. Assets above the root are immune (they render to `cdn.mdfly.dev/.../<hash>.<ext>`, no path in the URL). The v1 CLI skips outside-root files, so above-root keys are not produced in practice; the scheme is retained server-side for forward-compatibility (e.g. a future `--root` flag).
